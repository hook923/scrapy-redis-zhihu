...
class SpiderSimpleQueue(Base):
    ...
    def push(self, request):
        """Push a request"""
        self.server.lpush(self.key, request.url[37:])
    ...
    def pop(self, timeout=0):
        ...
        if url:
            try:
                url = url.decode()
                if "/followers" in url :
                    cb = getattr(self.spider, "parse_followers")
                elif "/followees" in url:
                    cb = getattr(self.spider, "parse_follows")
                elif "locations," in url:
                    cb = getattr(self.spider, "parse_user")
                elif "/answers" in url:
                    cb = getattr(self.spider, "parse_answers")
                elif "/questions" in url:
                    cb = getattr(self.spider, "parse_question")
                elif "/articles" in url:
                    cb = getattr(self.spider, "parse_article")
                else:
                    raise ValueError("Method not found in: %s( URL:%s )" % (self.spider, url))
                return Request(url="https://www.zhihu.com/api/v4/members/%s" % url, callback=cb)
